...
import static org.worldlisttrashcan.utils.IsVersion.Is1_12_1_16Server;
...
//        ConfigStringReplace(message.getConfig(),"&","§");
        ...
        pluginTitle = org.worldlisttrashcan.utils.Message.find("PluginTitle");
        ConfigStringReplace(org.worldlisttrashcan.utils.Message.getConfig(),"%PluginTitle%",pluginTitle);
//        message.saveData();
...
//            System.out.println("指定的字符串不在文件名中。");
    ...
    public static String find(String path){
        if(org.worldlisttrashcan.utils.Message.getConfig().getString(path)!=null&&!org.worldlisttrashcan.utils.Message.getConfig().getString(path).isEmpty()){
            return org.worldlisttrashcan.utils.Message.getConfig().getString(path);
        }else {
            if (!notFindMessageSlave.contains(path)) {
                consoleSay(org.worldlisttrashcan.utils.Message.find("NotFindMessageSlave").replace("%path%", path));
    ...
    public static void saveData() {
        try {
            ...
            Bukkit.getLogger().info(org.worldlisttrashcan.utils.Message.find("NotFindMessage"));
        }
    }
